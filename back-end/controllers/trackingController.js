...
exports.getPhanBoHocSinhTram = async (req, res) => {
  try {
    const phanBoHocSinh = await trackingService.getPhanBoHocSinhTram();
...
exports.getPhanBoTramXe = async (req, res) => {
  try {
    const phanBoTramXe = await trackingService.getPhanBoTramXe();
    res.json({ success: true, data: phanBoTramXe });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  } 
};
